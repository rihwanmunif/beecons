$(document).ready(async function () {
	try {
		const categoryBundlingResponse = await $.ajax({
			url: "https://lms.lazy2.codes/api/category-bundling",
			method: "GET",
			dataType: "json",
		});

		const response = await $.ajax({
			url: "https://lms.lazy2.codes/api/course-bundling",
			method: "GET",
			dataType: "json",
		});
		...
		$("#courses .courses-bundling .courses-bundling-rekomendasi").html(
			generateBundles(rekomendasi)
		);
		...
		function setBundles(tag = 0) {
			$(`.courses-bundlings .tags .item[data-category_bundling_id="${tag}"]`)
				.addClass("active")
				.siblings()
				.removeClass("active");
			...
			if (tag === 0) {
				...
				result = response.filter(
					(item) => item.bundling[0].category_bundling_id === tag.toString()
				);
			}
