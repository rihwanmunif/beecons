$(document).ready(async function () {
	...
	try {
		const tagsResponse = await $.ajax({
			url: "https://lms.lazy2.codes/api/type_tag",
			method: "GET",
			dataType: "json",
		});

		const categoryResponse = await $.ajax({
			url: "https://lms.lazy2.codes/api/category",
			method: "GET",
			dataType: "json",
		});
		...
		$("#courses  #tab-courses-1 .tags .item").on("click", function (e) {
			...
			generateListCourse(
				...
				$("#courses-engineering"),
				"1",
				localStorage.getItem("current-tag-engineering"),
				localStorage.getItem("current-category-engineering")
			);
		});
		...
		$("#courses  #tab-courses-2 .tags .item").on("click", function (e) {
			...
			generateListCourse(
				...
				$("#courses-it"),
				"2",
				localStorage.getItem("current-tag-it"),
				localStorage.getItem("current-category-it")
			);
		});
		...
		async function generateListCourse(courses, element, type, tag, category) {
			let currentTag = $(
				`#courses #tab-courses-${type} .tags .item[data-tag_id="${tag}"]`
			).html();
			...
			$(
				`#courses #tab-courses-${type} .sub-tags .item[data-category_id=${category}]`
			).addClass("active");

			let coursesByType = courses.filter(
				(course) => course.type[0].type_id === type
			);
			let coursesBytag = coursesByType.filter((course) =>
				course.tag.map((tag) => tag.tag_id).includes(tag)
			);
			let coursesByCategory = coursesBytag.filter(
				(course) => course.category[0].category_id === category
			);
			...
			if (Cookies.get("access_token")) {
				...
				try {
					const res = await $.ajax({
						url: `https://lms.lazy2.codes/api/user-course`,
						method: "GET",
						dataType: "json",
						headers: {
                              ...
                              <div class="card-course">
                                  ...
                                  <div class="card-course-button">
                                      ${(() => {
                                                  ...
                                                  <button value=${
																										course.course_id
																									} class="button-secondary add-cart"><i class="fa-solid fa-cart-shopping"></i></button>
                                              `;
																				} else {
																					return `
                                                  <a href="${`/course/${course.course_id}`}">
		...
		function handleAddCart() {
			return $(".add-cart").on("click", async function () {
				...
				try {
					const res = await $.ajax({
						url: `https://lms.lazy2.codes/api/cart/create/course/${course_id}`,
						method: "POST",
						dataType: "json",
						headers: {
							Authorization: "Bearer " + Cookies.get("access_token"),
					...
					const { item } = await $.ajax({
						url: "https://lms.lazy2.codes/api/cart",
						method: "GET",
						dataType: "json",
						headers: {
							Authorization: "Bearer " + Cookies.get("access_token"),
